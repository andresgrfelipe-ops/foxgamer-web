(() => {
 ...
 function render(){
  ...
  const matches=cards.filter(c=>c.dataset.condition===condition.value&&(!platform.value||c.dataset.platform===platform.value)&&terms.every(t=>norm(c.dataset.name).includes(t)));
  ...
  count.textContent=matches.length+' juegos · '+condition.value+(platform.value?' · '+platform.value:'');more.hidden=matches.length<=limit;get('gallery-empty').hidden=matches.length>0;
 }
 function update(){limit=12;render();}
 function reset(){query.value='';platform.value='';condition.value='Nuevo';sort.value='featured';update();}
