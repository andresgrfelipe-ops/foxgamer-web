...
import {validateStore, imageIdentity} from './catalog.mjs';
...
export async function auditImages(store, root = process.cwd()) {
  ...
  for (const p of withImage) {
    if (checked.has(p.name + p.image)) continue;
    ...
    const identity = imageIdentity(proof);
    if (hashes.has(hash) && hashes.get(hash) !== identity) throw new Error('Foto duplicada entre modelos: ' + p.name);
    hashes.set(hash, identity);
    checked.add(p.name + p.image);
  }
