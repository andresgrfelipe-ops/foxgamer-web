...
test('accesorios distinguen compatibilidad y marcan como ilustración los modelos ambiguos',()=>{
 ...
 for(const name of ['Monitores','Diademas','Soporte para volante y pedales'])assert(store.products.filter(p=>p.name===name).every(p=>p.image&&store.verifiedImages.some(v=>v.name===name&&v.image===p.image&&v.kind==='illustration')));
