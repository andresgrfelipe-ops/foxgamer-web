...
const ctx={};vm.runInNewContext(fs.readFileSync('assets/full-game-model.js','utf8'),ctx);const {mergeEntries,selectEntries}=ctx.FoxGameDirectory;
...
test('incluye todas las entradas del directorio real y conserva las 47 fichas comerciales',()=>{
 ...
 const all=mergeEntries([...groups.values()],d.games);assert.equal(selectEntries(all,{verified:true}).length,47);assert(all.length>=d.games.length);
